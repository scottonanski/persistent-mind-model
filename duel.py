...
try:
    agent_a.set_drift_params(
        ...
        notes_append="Increase delta for exploration & calibration.",
    )
    agent_b.set_drift_params(
        max_delta_per_reflection=0.03,
        notes_append="Increase delta for exploration & calibration.",
    )
except Exception:
...
def agent_step(
    agent_mgr: SelfModelManager,
    other_name: str,
    incoming_text: str,
    stimulator: CadenceStimulator,
):
    if incoming_text:
        agent_mgr.add_event(
            summary=f"Heard from {other_name}: {incoming_text}", effects=[]
        )
    ...
    agent_mgr.add_event(
        summary=f"Stimulus: {stimulus['summary']}", effects=stimulus.get("effects", [])
    )
    ...
    for attempt in range(max_retries):
        try:
            ...
            print(
                f"   ⚠️  LLM call failed (attempt {attempt + 1}/{max_retries}): {type(e).__name__}: {str(e)}"
            )
    ...
    try:
        ...
        if any(abs(v) >= 0.0001 for v in drift_applied.values()):
            ...
            try:
                patterns = (
                    getattr(agent_mgr.model, "self_knowledge", agent_mgr.model).get(
                        "behavioral_patterns"
                    )
                    if isinstance(agent_mgr.model, dict)
                    else agent_mgr.model.self_knowledge.behavioral_patterns
                )
                ...
                if ct and getattr(ct, "commitments", None):
                    ...
                    closed = sum(
                        1
                        for c in ct.commitments.values()
                ...
                print(
                    f"   📊 Evidence: exp={exp_count}(+{exp_delta}), align={align_count}(+{align_delta}), close_rate={close_rate:.1%}(+{close_rate_delta:.2f}) → weight={evidence_weight:.2f}"
                )
...
if __name__ == "__main__":
    ...
    for agent_name, agent_mgr in [("Agent A", agent_a), ("Agent B", agent_b)]:
        try:
            ...
            delta_final = {
                k: round(new_traits[k] - prev_traits[k], 6) for k in prev_traits
            }
            if any(abs(v) >= 0.0001 for v in delta_final.values()):
                drift_str = ", ".join(
                    [
                        f"{k}:{v:+.3f}"
                        for k, v in delta_final.items()
                        if abs(v) >= 0.0001
                    ]
                )
                ...
                try:
                    ...
                    evidence_weight = min(1, signals / 3)
                    print(
                        f"{agent_name} 📊 Evidence: exp={exp_count}(+{exp_delta}), align={align_count}(+{align_delta}), close_rate={close_rate:.1%}(+{close_rate_delta:.2f}) → weight={evidence_weight:.2f}"
                    )
                except Exception as ex:
                    print(
                        f"{agent_name} ⚠️ Evidence logging failed: {type(ex).__name__}"
                    )
            else:
    ...
    for agent_name, agent_mgr in [("Agent A", agent_a), ("Agent B", agent_b)]:
        try:
            ...
            if ct and getattr(ct, "commitments", None):
                open_commitments = [
                    cid
                    for cid, c in ct.commitments.items()
                ...
                print(
                    f"{agent_name}: {len(open_commitments)} open, {len(closed_commitments)} closed, close_rate={close_rate:.1%}"
                )
                if len(open_commitments) > 0:
                    print(
                        f"  Open: {open_commitments[:5]}{'...' if len(open_commitments) > 5 else ''}"
                    )
            else:
                print(f"{agent_name}: No commitment tracker available")
        except Exception as e:
            print(
                f"{agent_name}: commitment summary error - {type(e).__name__}: {str(e)}"
            )

    # Session-level Big5 delta summary
    try:
        print("\nSession ΔBig5:")
        for agent_name, agent_mgr, start_b5 in [
            ("Agent A", agent_a, start_b5_a),
            ("Agent B", agent_b, start_b5_b),
        ]:
            end_b5 = _big5_snapshot(agent_mgr)
            session_delta = {k: round(end_b5[k] - start_b5[k], 6) for k in start_b5}
            delta_str = ", ".join(
                [f"{k}:{v:+.3f}" for k, v in session_delta.items() if abs(v) >= 0.0001]
            )
