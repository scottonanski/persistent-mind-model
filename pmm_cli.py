...
def create_manager(model_path: str, enable_next_stage: bool = True) -> EnhancedSelfModelManager:
    """Create enhanced PMM manager instance."""
    try:
        return EnhancedSelfModelManager(
            model_path=model_path,
            enable_next_stage=enable_next_stage
        )
...
def cmd_import_identity(args):
    ...
    try:
        result = manager.import_identity(
...
def cmd_verify_integrity(args):
    ...
    print(f"🔒 Lockpoints: {result['lockpoints_valid']}/{len(result['lockpoints'])} valid")
...
def cmd_archive_status(args):
    ...
    if args.trigger_archival:
        ...
        if "error" in result:
            ...
            print(f"✅ Archived {result['archived_tokens']} tokens in {result['clusters_created']} clusters")
...
Examples:
  ...
  pmm-cli verify-integrity --next-stage
    ...
    # Recall Memories
    recall_parser = subparsers.add_parser("recall", help="Recall memories by semantic cue")
    ...
    generate_parser.add_argument("--max-tokens", type=int, default=1024, help="Max tokens")
    ...
    export_parser.add_argument("--no-archives", dest="include_archives", action="store_false", help="Exclude archives")
    ...
    import_parser.add_argument("--merge", action="store_true", help="Merge with existing identity")
    import_parser.set_defaults(func=cmd_import_identity)
    
    # Verify Integrity
    verify_parser = subparsers.add_parser("verify-integrity", help="Verify PMM integrity")
    ...
    archive_parser.add_argument("--trigger", dest="trigger_archival", action="store_true", help="Trigger manual archival")
