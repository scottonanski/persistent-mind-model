...
class HotTickValidator:
    ...
    def test_reflection_in_hot_context(self, memory_manager):
        ...
        try:
            result = memory_manager._auto_reflect(
                active_model_config=ModelConfig().model_dump(), force_reflect=True
            )
    ...
    def run_scenario(self, scenario_name: str, target_hot_strength: float = 0.7):
        ...
        print(
            f"Baseline - GAS: {baseline['gas']:.3f}, Close Rate: {baseline['close_rate']:.3f}, Hot Strength: {baseline['hot_strength']:.3f}"
        )

        # Create hot scenario
        hot_metrics = self.create_hot_scenario(memory_manager, scenario_name)
        print(
            f"Hot Context - GAS: {hot_metrics['gas']:.3f}, Close Rate: {hot_metrics['close_rate']:.3f}, Hot Strength: {hot_metrics['hot_strength']:.3f}"
        )
        ...
        scenario_result = {
            ...
            "success_criteria": {
                ...
                "reflection_accepted": reflection_result.get(
                    "reflection_accepted", False
                ),
                "target_hot_strength_met": hot_metrics["hot_strength"]
                >= target_hot_strength,
            },
        }
    ...
    def generate_report(self):
        ...
        report = {
            ...
            "summary": {
                ...
                "successful_scenarios": sum(
                    1 for r in self.results if all(r["success_criteria"].values())
                ),
                "hot_contexts_achieved": sum(
                    1
                    for r in self.results
                    if r["success_criteria"]["achieved_hot_context"]
                ),
                "reflections_accepted": sum(
                    1
                    for r in self.results
                    if r["success_criteria"]["reflection_accepted"]
                ),
            },
        }
        ...
        # Save report
        report_file = (
            Path(__file__).parent / f"hot_tick_validation_{int(time.time())}.json"
        )
...
def main():
    ...
    success_rate = (
        summary["successful_scenarios"] / summary["total_scenarios"]
