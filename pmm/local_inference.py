...
class OllamaProvider(BaseInferenceProvider):
    ...
    def is_available(self) -> bool:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                return any(
                    model.get("name", "").startswith(self.model) for model in models
                )
            return False
        except Exception:
            return False
    ...
    def get_model_info(self) -> Dict[str, Any]:
        ...
        try:
            response = requests.post(
                f"{self.base_url}/api/show", json={"name": self.model}, timeout=5
            )
            if response.status_code == 200:
    ...
    def generate_text(self, prompt: str, **kwargs) -> InferenceResult:
        ...
        try:
            ...
            return InferenceResult(
                ...
                provider="llamacpp",
                model=(
                    os.path.basename(self.model_path) if self.model_path else "unknown"
                ),
                success=False,
    ...
    def get_model_info(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "tokenizer_vocab_size": (
                len(self.pipeline.tokenizer) if self.pipeline.tokenizer else 0
            ),
        }
...
class LocalInferenceEngine:
    ...
    def _determine_provider_order(self) -> List[str]:
        ...
        # Fallback provider
        if (
            self.config.api_fallback_enabled
            and self.config.fallback_provider not in order
            and self.providers[self.config.fallback_provider].is_available()
        ):
