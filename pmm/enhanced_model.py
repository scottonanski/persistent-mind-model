...
from .model import (
    CoreIdentity, Personality, NarrativeIdentity, 
    Metrics, DriftConfig, MetaCognition,
    Event, Thought, Insight
)
...
class EnhancedSelfKnowledge:
    ...
    memory_tokens: Dict[str, MemoryToken] = field(default_factory=dict)  # token_id -> token
    memory_chain: MemoryChain = field(default_factory=MemoryChain)
    
    # Archive system
    archives: Dict[str, MemoryArchive] = field(default_factory=dict)  # archive_id -> archive
...
class EnhancedPersistentMindModel:
    ...
    inception_moment: str = field(default_factory=lambda: datetime.utcnow().strftime("%Y-%m-%dT%H:%M:%SZ"))
    ...
    def upgrade_from_legacy(self, legacy_model) -> None:
        ...
        self.self_knowledge.behavioral_patterns = legacy_model.self_knowledge.behavioral_patterns
        self.self_knowledge.autobiographical_events = legacy_model.self_knowledge.autobiographical_events
        ...
        self.metrics.reflection_cadence_days = legacy_model.metrics.reflection_cadence_days
