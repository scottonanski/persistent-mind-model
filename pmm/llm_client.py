...
class LLMClient:
    ...
    def chat(
        ...
        try:
            ...
            if response.usage:
                ...
                logger.debug(
                    f"Tokens used: {tokens_used}, Total: {self.total_tokens_used}"
                )
    ...
    def _make_request(
        ...
        logger.debug(
            f"Making LLM request: model={self.config.model}, temp={temperature}"
        )
    ...
    async def chat_async(
        ...
        return await loop.run_in_executor(
            None, self.chat, system, user, temperature, max_tokens
        )
...
class BatchLLMClient:
    ...
    async def process_batch(self, requests: List[Dict[str, str]]) -> List[str]:
        ...
        for i in range(0, len(requests), self.batch_size):
            ...
            tasks = [
                self.client.chat_async(system=req["system"], user=req["user"])
                for req in batch
            ]
