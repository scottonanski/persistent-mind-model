...
class ArchiveEngine:
    ...
    def should_trigger_archival(
        self, active_tokens: Dict[str, MemoryToken], active_token_ids: List[str]
    ) -> bool:
        """
        ...
        for token_id in active_token_ids:
            ...
            if (
                token_date < cutoff_date
                and token.amplitude < self.config.amplitude_threshold
            ):
                old_low_tokens += 1
    ...
    def identify_archival_candidates(
        ...
        for token_id in active_token_ids:
            ...
            should_archive = token.amplitude < self.config.amplitude_threshold or (
    ...
    def create_thematic_clusters(
        self, tokens_to_archive: Dict[str, MemoryToken]
    ) -> List[ThemeCluster]:
        """
    ...
    def archive_clusters(
        self, clusters: List[ThemeCluster], tokens_to_archive: Dict[str, MemoryToken]
    ) -> MemoryArchive:
        """
        ...
        for cluster in clusters:
            archive.add_theme_cluster(
                cluster.theme_label, cluster.token_ids, cluster.centroid_summary
            )
    ...
    def create_identity_lockpoint(
        self, pmm_model: Any, chain_position: int
    ) -> IdentityLockpoint:
        """
        ...
        lockpoint = IdentityLockpoint(
            ...
            chain_hash=(
                pmm_model.self_knowledge.memory_chain.tokens[-1].content_hash
                if pmm_model.self_knowledge.memory_chain.tokens
                else "empty"
            ),
        )
        ...
        lockpoint_path = os.path.join(
            self.config.archive_base_path, f"lockpoint_{lockpoint.lockpoint_id}.json"
        )
    ...
    def _create_single_cluster(
        ...
        avg_salience = sum(tokens_to_archive[tid].salience for tid in token_ids) / len(
            token_ids
        )

        # Create summary
        _summaries = [tokens_to_archive[tid].summary for tid in token_ids]
    ...
    def _build_theme_cluster(
        ...
        tokens_to_archive: Dict[str, MemoryToken],
    ) -> ThemeCluster:
        ...
        # Average salience
        avg_salience = sum(token.salience for token in cluster_tokens) / len(
            cluster_tokens
        )
...
class ArchiveRetriever:
    ...
    def retrieve_tokens_by_theme(
        self, archive: MemoryArchive, theme: str
    ) -> List[Dict[str, Any]]:
    ...
    def _load_from_sqlite(self, db_path: str) -> Dict[str, Any]:
        ...
        return {"metadata": metadata, "clusters": clusters, "tokens": tokens}
