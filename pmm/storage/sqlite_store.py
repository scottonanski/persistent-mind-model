...
class SQLiteStore:
    ...
    def append_event(self, kind: str, content: str, meta: Dict[str, Any], hsh: str, prev: Optional[str]):
        """Append new event to the chain."""
        self.conn.execute(
            "INSERT INTO events(ts,kind,content,meta,prev_hash,hash) VALUES(?,?,?,?,?,?)",
            (time.strftime("%Y-%m-%d %H:%M:%S"), kind, content, json.dumps(meta, ensure_ascii=False), prev, hsh)
        )
        self.conn.commit()

    def all_events(self):
        """Get all events in chronological order."""
        return list(self.conn.execute("SELECT id,ts,kind,content,meta,prev_hash,hash FROM events ORDER BY id"))
    
    def recent_events(self, limit: int = 10):
        """Get recent events for context."""
        return list(self.conn.execute("SELECT id,ts,kind,content,meta,prev_hash,hash FROM events ORDER BY id DESC LIMIT ?", (limit,)))
