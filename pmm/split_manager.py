...
class SplitModelManager:
    ...
    def __init__(
        ...
        self.events_path = Path(events_path or model_path.replace('.json', '.events.jsonl'))
        self.insights_path = Path(insights_path or model_path.replace('.json', '.insights.jsonl'))
        self.thoughts_path = Path(thoughts_path or model_path.replace('.json', '.thoughts.jsonl'))
    ...
    def add_event(self, summary: str, effects: Optional[List[dict]] = None, *, etype: str = "experience") -> Event:
        ...
        with self.lock:
            ...
            if len(self.model.self_knowledge.autobiographical_events) > self.recent_events_limit:
                self.model.self_knowledge.autobiographical_events = \
                    self.model.self_knowledge.autobiographical_events[-self.recent_events_limit:]
            
            self._save_model_only(self.model)
            return event
    
    def add_insight(self, content: str, summary: Optional[str] = None, commitment: Optional[str] = None) -> Insight:
        ...
        with self.lock:
            ...
            insight_dict = {
                "id": in_id, "t": ts, "content": content,
                "summary": summary, "commitment": commitment,
                "references": {}
            }
            ...
            if len(self.model.self_knowledge.insights) > self.recent_insights_limit:
                self.model.self_knowledge.insights = \
                    self.model.self_knowledge.insights[-self.recent_insights_limit:]
    ...
    def add_thought(self, content: str, trigger: str = "") -> Thought:
        ...
        with self.lock:
            ...
            # Append to log
            thought_dict = {
                "id": th_id, "t": ts, "content": content, "trigger": trigger
            }
            ...
            if len(self.model.self_knowledge.thoughts) > self.recent_thoughts_limit:
                self.model.self_knowledge.thoughts = \
                    self.model.self_knowledge.thoughts[-self.recent_thoughts_limit:]
    ...
    def _extract_commitment(self, content: str) -> Optional[str]:
        ...
        for line in lines:
            line = line.strip()
            if any(starter in line for starter in ['i will', 'next:', 'i plan to', 'i commit to']):
                return line.capitalize()
        return None
    
    def _track_commitment(self, commitment: str, insight_id: str) -> None:
        """Track commitment in future_scripts.goals."""
        goal_id = f"goal_{len(self.model.narrative_identity.future_scripts.goals) + 1}"
