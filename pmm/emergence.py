...
class EmergenceAnalyzer:
    ...
    def pmmspec_match(self, text: str) -> float:
        ...
        pmm_keywords = [
            'persistent mind model', 'pmm', 'personality', 'memory', 'commitment',
            'self-model', 'identity', 'cross-session', 'verifiable', 'evolution'
        ]
    ...
    def get_recent_events(self, kind: str = "response", limit: int = 5) -> List[EmergenceEvent]:
    ...
    def compute_scores(self, window: int = 5) -> Dict[str, Any]:
        ...
        GAS = (0.5 * (1.0 if any(exp_detects) else 0.0) +
               0.3 * novelty +
               0.2 * commit_rate)
    ...
    def detect_stage(self, IAS: float, GAS: float, exp_detect: bool, 
                    pmmspec: float, selfref: float) -> str:
