...
class EmbeddingCache:
    ...
    def set_embedding(self, text_id: str, embedding: np.ndarray, metadata: Dict[str, Any] = None):
        ...
        self.metadata[text_id] = metadata or {'created_at': datetime.utcnow().isoformat()}
...
class RecallEngine:
    ...
    def recall_memories(self, 
                       ...
                       max_results: Optional[int] = None) -> List[RecallResult]:
        """
        ...
        active_results = self._search_active_memory(
            cue, cue_embedding, self_knowledge.memory_tokens, self_knowledge.active_token_ids
        )
        ...
        if len(active_results) < max_results:
            archive_results = self._search_archives(
                cue, cue_embedding, self_knowledge.archives, max_results - len(active_results)
            )
        ...
        if len(results) < max_results:
            for archive_id, archive in self_knowledge.archives.items():
                if theme in archive.themes:
                    archive_tokens = self.archive_retriever.retrieve_tokens_by_theme(archive, theme)
    ...
    def recall_by_temporal_range(self,
                                start_time: str,
                                end_time: str,
                                self_knowledge: EnhancedSelfKnowledge,
                                max_results: Optional[int] = None) -> List[RecallResult]:
    ...
    def verify_recalled_memory(self, 
                              result: RecallResult,
    ...
    def _search_active_memory(self, 
                             cue: str,
                             cue_embedding: Optional[np.ndarray],
                             memory_tokens: Dict[str, MemoryToken],
                             active_token_ids: List[str]) -> List[RecallResult]:
    ...
    def _search_archives(self,
                        ...
                        max_results: int) -> List[RecallResult]:
        ...
        for archive_id, archive in archives.items():
            try:
                ...
                for token_id, token_data in archive_data["tokens"].items():
                    ...
                    if similarity >= self.config.similarity_threshold * 0.8:  # Lower threshold for archives
                        # Reconstruct token
                        token = self._reconstruct_token_from_archive(token_data)
    ...
    def _calculate_similarity_with_data(self,
                                       cue: str,
                                       cue_embedding: Optional[np.ndarray],
                                       token_data: Dict[str, Any]) -> float:
    ...
    def _calculate_keyword_similarity(self, cue: str, token_text: str, token: MemoryToken) -> float:
    ...
    def _calculate_keyword_similarity_with_data(self, cue: str, token_data: Dict[str, Any]) -> float:
    ...
    def _reconstruct_token_from_archive(self, token_data: Dict[str, Any]) -> MemoryToken:
    ...
    def _apply_recall_effects(self, 
