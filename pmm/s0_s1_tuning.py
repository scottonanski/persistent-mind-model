...
class S0S1Tuner:
    ...
    def evaluate_stage_transition(
        self, ias: float, gas: float, event_count: int
    ) -> str:
        """
        ...
        if (
            self.current_stage == "S0"
            and ias >= self.config.s1_threshold_ias
            and gas >= self.config.s1_threshold_gas
        ):
        ...
        # S1 -> S0 transition (with hysteresis)
            ...
            ias < self.config.s0_threshold_ias - self.config.transition_hysteresis
            or gas < self.config.s0_threshold_gas - self.config.transition_hysteresis
        ):
            self.current_stage = "S0"
    ...
    def _record_transition(
        self, from_stage: str, to_stage: str, ias: float, gas: float, event_count: int
    ) -> None:
