...
class AdaptiveDirectiveClassifier:
    ...
    def classify_with_context(self, text: str, context: ConversationContext) -> Tuple[str, float]:
        """
        ...
        meta_score = self._calculate_pattern_score(text_lower, self.meta_principle_patterns)
        principle_score = self._calculate_pattern_score(text_lower, self.principle_patterns)
        commitment_score = self._calculate_pattern_score(text_lower, self.commitment_patterns)
    ...
    def _get_context_modifier(self, context: ConversationContext, directive_type: str) -> float:
        ...
        if directive_type == "meta-principle":
            if any(word in user_lower for word in ["permanent", "always", "identity", "core"]):
                modifier += 0.3
        elif directive_type == "principle":
            if any(word in user_lower for word in ["guideline", "approach", "generally"]):
                modifier += 0.3
        elif directive_type == "commitment":
            if any(word in user_lower for word in ["commit", "will do", "promise", "deliver"]):
                modifier += 0.3
        
        # Conversation phase
        if hasattr(context, 'conversation_phase'):
            if context.conversation_phase == "commitment" and directive_type == "commitment":
                modifier += 0.2
            elif context.conversation_phase == "evolution" and directive_type == "meta-principle":
                modifier += 0.2
