...
class PMMRuntime:
    ...
    def ask(self, user_text: str, history: List[Message] | None = None, max_tokens: int = 512) -> str:
    ...
    def _build_pmm_context(self, user_text: str) -> Dict:
        ...
        patterns = getattr(self.pmm_manager.model.self_knowledge, 'behavioral_patterns', {})
        
        return {
            'personality': big5_traits,
            'commitments': open_commitments,
            'recent_events': [{'summary': e.summary, 'timestamp': e.timestamp} for e in recent_events],
    ...
    def _build_pmm_messages(self, pmm_context: Dict, user_text: str, history: List[Message]) -> List[Dict]:
...
Be concise but genuine to your persistent identity."""
    ...
    def _evaluate_commitments(self, reply: str) -> None:
        ...
        if recent_commits:
            ...
            recent_responses = [e[3] for e in all_events if e[2] in ["response", "reflection"]][-3:]
            
            # Autonomously evaluate if commitment is fulfilled
            if self.auto_evaluate(commit_content, recent_responses):
                self._append("evidence", f"Autonomous completion assessment: {commit_content[:100]}...", 
                           {"commit_ref": commit_hash, "evidence_type": "autonomous_assessment"})
            ...
            if pattern in user_lower:
                ...
                try:
                    ...
                    for i in range(len(words) - len(pattern_words) + 1):
                        if all(words[i+j].lower() == pattern_words[j] for j in range(len(pattern_words))):
                    ...
                    if pattern_idx and pattern_idx < len(words):
                        ...
                        if len(potential_name) > 1 and potential_name.isalpha():
                            ...
                            print(f"🔄 PMM identity updated: Name is now '{potential_name}'")
                            break
    ...
    def micro_reflect(self) -> str:
        ...
        self._append("prompt", prompt, {"role": "system", "model": self.model_name, "tag": "micro_reflection"})
        reply = self.adapter.generate([{"role": "user", "content": prompt}], max_tokens=80)
        self._append("reflection", reply, {"role": "assistant", "model": self.model_name, "tag": "micro_reflection"})
        ...
        if recent_commits and any(keyword in reply.lower() for keyword in ["completed", "done", "finished", "accomplished"]):
            last_commit = recent_commits[-1]
            commit_hash = last_commit[6]  # hash field
            self._append("evidence", f"Self-reflection indicates completion: {reply}", 
                       {"commit_ref": commit_hash, "evidence_type": "self_assessment"})
        
        return reply

    def auto_evaluate(self, commitment_content: str, recent_history: List[str], threshold: float = 0.6) -> bool:
        ...
        try:
            ...
            completion_keywords = ["completed", "done", "finished", "achieved", "accomplished", "fulfilled"]
            keyword_match = any(keyword in combined_history for keyword in completion_keywords)
