...
class PatternContinuityManager:
    ...
    def __init__(
        ...
        self.min_references = (
            min_references or min_reference_count
    ...
    def track_pattern(
        self, pattern_type: str, content: str, metadata: Optional[Dict[str, Any]] = None
    ) -> None:
        """
    ...
    def analyze_continuity(self, window_size: int = 10) -> Dict[str, Any]:
        ...
        """
        recent_events = (
            self.pattern_history[-window_size:] if self.pattern_history else []
        )
        ...
        continuity_score = (
            dominant_pattern_count / total_events if total_events > 0 else 0.0
        )

        # Pattern diversity (Shannon entropy-like measure)
        pattern_diversity = (
            len(pattern_counts) / total_events if total_events > 0 else 0.0
        )
        ...
        return {
            ...
            "average_confidence": sum(e.confidence for e in recent_events)
            / len(recent_events),
        }
    ...
    def _update_pattern_detection(self, event: PatternEvent) -> None:
        ...
        pattern_key = (
            f"{event.pattern_type}_{self._get_content_signature(event.content)}"
        )
    ...
    def _get_dominant_patterns(self) -> List[str]:
        ...
        for event in self.pattern_history[-20:]:  # Look at recent history
            pattern_counts[event.pattern_type] = (
                pattern_counts.get(event.pattern_type, 0) + 1
            )
        ...
        return [
            ...
            if count >= self.min_reference_count
        ]
    ...
    def enhance_context_with_continuity(self, original_context: str) -> str:
        ...
        if commitments:
            ...
            continuity_insights.append(
                f"Recent commitments: {len(commitments)} active ({commitment_refs})"
            )
        ...
        if reflections:
            ...
            continuity_insights.append(
                f"Reflection frequency: {len(reflections)} insights ({insight_refs})"
            )
        ...
        if evidence:
            ...
            continuity_insights.append(
                f"Evidence tracking: {len(evidence)} completions ({evidence_refs})"
            )
        ...
        if strong_patterns:
            pattern_summary = ", ".join(
                [p["pattern_type"] for p in strong_patterns[:3]]
            )
