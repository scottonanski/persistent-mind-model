...
class TokenizationEngine:
    ...
    def tokenize_event(self, event: Event, content: Optional[str] = None) -> MemoryToken:
        """
        ...
        content_hash = self.chain.add_token(token, full_content)
    ...
    def tokenize_thought(self, thought: Thought) -> MemoryToken:
        ...
        token = MemoryToken(
            ...
            summary=thought.content[:200] + "..." if len(thought.content) > 200 else thought.content,
            amplitude=0.8,  # Thoughts start slightly less active than events
            phase=0.0      # Neutral phase for thoughts
        )
        
        # Add to chain
        content_hash = self.chain.add_token(token, thought.content)
    ...
    def tokenize_insight(self, insight: Insight) -> MemoryToken:
        ...
        token = MemoryToken(
            ...
            summary=insight.content[:200] + "..." if len(insight.content) > 200 else insight.content,
            amplitude=1.0,  # Insights start fully active
            phase=1.57     # π/2 radians for "enlightenment" semantic position
        )
        
        # Add to chain
        content_hash = self.chain.add_token(token, insight.content)
    ...
    def tokenize_custom_content(self, 
                               content: str, 
                               event_type: str = "custom",
                               salience: float = 0.5,
                               valence: float = 0.5,
                               tags: Optional[List[str]] = None) -> MemoryToken:
        """
        ...
        # Add to chain
        content_hash = self.chain.add_token(token, content)
    ...
    def get_chain_statistics(self) -> Dict[str, Any]:
        ...
        chain_stats.update({
            ...
            "archived_tokens": len([t for t in self.self_knowledge.memory_tokens.values() if t.archived])
        })
    ...
    def create_identity_lockpoint(self, pmm_model: Any) -> IdentityLockpoint:
        ...
        lockpoint = IdentityLockpoint(
            ...
            chain_hash=self.chain.tokens[-1].content_hash if self.chain.tokens else "empty"
        )
        ...
        if len(self.self_knowledge.lockpoints) > max_lockpoints:
            self.self_knowledge.lockpoints = self.self_knowledge.lockpoints[-max_lockpoints:]
    ...
    def _compute_semantic_phase(self, event: Event) -> float:
        ...
        # Combine components
        phase = (valence_component + arousal_component + type_adjustment) % (2 * 3.14159)
    ...
    def _compute_content_phase(self, content: str) -> float:
        ...
        positive_words = ["good", "great", "excellent", "happy", "success", "achieve", "love", "joy"]
        negative_words = ["bad", "terrible", "sad", "fail", "hate", "anger", "fear", "worry"]
...
class ChainVerifier:
    ...
    def verify_full_chain(chain: MemoryChain) -> Tuple[bool, List[str], Dict[str, Any]]:
        ...
        if chain.tokens:
            ...
            timestamps = [datetime.fromisoformat(t.created_at.replace('Z', '+00:00')) for t in chain.tokens]
        ...
        for i, token in enumerate(chain.tokens):
            if token.amplitude > avg_amplitude * 2:
                anomalies.append({
                    ...
                    "details": {"position": i, "amplitude": token.amplitude, "average": avg_amplitude}
                })
        ...
        for i in range(1, len(timestamps)):
            if timestamps[i] < timestamps[i-1]:
                anomalies.append({
                    ...
                    "details": {"position": i, "current": timestamps[i].isoformat(), "previous": timestamps[i-1].isoformat()}
                })
