...
class MetaReflectionAnalyzer:
    ...
    def analyze_reflection_patterns(self, recent_reflections: List[Dict[str, Any]], 
        ...
        for i, text in enumerate(reflection_texts):
            ...
            novelty = self.semantic_analyzer.semantic_novelty_score(text, previous_texts)
        ...
        clusters = self.semantic_analyzer.cluster_similar_texts(reflection_texts, similarity_threshold=0.7)
        
        # Generate insights
        patterns = self._identify_patterns(reflection_texts, clusters)
        recommendations = self._generate_recommendations(quality_scores, novelty_scores, patterns)
        
        return {
            "total_reflections": len(windowed_reflections),
            "avg_quality": sum(quality_scores) / len(quality_scores) if quality_scores else 0.0,
            "novelty_trend": sum(novelty_scores) / len(novelty_scores) if novelty_scores else 0.0,
    ...
    def _assess_reflection_quality(self, reflection_text: str) -> float:
        ...
        specific_indicators = [
            'event', 'commitment', 'evidence', 'pattern', 'behavior',
            'goal', 'strategy', 'learning', 'improvement', 'challenge'
        ]
        specificity = sum(1 for indicator in specific_indicators if indicator in reflection_text.lower())
        score += min(0.3, specificity * 0.05)
        
        # Self-reference indicators (meta-cognitive awareness)
        self_ref_indicators = ['i noticed', 'i realized', 'i learned', 'my approach', 'my pattern']
        self_refs = sum(1 for indicator in self_ref_indicators if indicator in reflection_text.lower())
        score += min(0.2, self_refs * 0.1)
        
        # Future-oriented planning
        future_indicators = ['next', 'will', 'plan', 'intend', 'goal', 'aim']
        future_refs = sum(1 for indicator in future_indicators if indicator in reflection_text.lower())
        score += min(0.2, future_refs * 0.05)
        
        return min(1.0, score)
    
    def _calculate_duplicate_rate(self, novelty_scores: List[float], threshold: float = 0.2) -> float:
    ...
    def _identify_patterns(self, reflection_texts: List[str], clusters: List[List[int]]) -> List[Dict[str, Any]]:
        ...
        if len(reflection_texts) >= 3:
            patterns.append({
                ...
                "avg_per_week": len(reflection_texts) * 7 / 7,  # Assuming 7-day window
            })
    ...
    def _generate_recommendations(self, quality_scores: List[float], novelty_scores: List[float], 
        ...
        avg_quality = sum(quality_scores) / len(quality_scores) if quality_scores else 0.0
        if avg_quality < 0.5:
            recommendations.append("Focus on more specific, detailed reflections with concrete examples")
        
        # Novelty-based recommendations
        avg_novelty = sum(novelty_scores) / len(novelty_scores) if novelty_scores else 0.0
        if avg_novelty < 0.3:
            recommendations.append("Explore new perspectives and avoid repeating similar insights")
        
        # Pattern-based recommendations
        repeated_themes = [p for p in patterns if p.get("type") == "repeated_theme"]
        if len(repeated_themes) > 2:
            recommendations.append("Consider diversifying reflection topics to avoid repetitive themes")
        
        # Frequency recommendations
        if len(quality_scores) < 2:
            recommendations.append("Increase reflection frequency for better self-awareness development")
        elif len(quality_scores) > 10:
            recommendations.append("Consider focusing on quality over quantity in reflections")
    ...
    def generate_meta_insight(self, pattern_analysis: Dict[str, Any]) -> Optional[str]:
        ...
        if avg_quality > 0.7:
            insights.append("My reflections show high quality with specific, actionable insights")
        ...
        if novelty_trend > 0.7:
            insights.append("I'm consistently generating novel insights and avoiding repetition")
        elif novelty_trend < 0.3:
            insights.append("I notice I'm repeating similar themes - I should explore new perspectives")
        
        # Pattern insight
        if pattern_analysis["patterns"]:
            repeated_themes = [p for p in pattern_analysis["patterns"] if p.get("type") == "repeated_theme"]
            if repeated_themes:
                insights.append(f"I tend to focus on {len(repeated_themes)} recurring themes in my reflections")
