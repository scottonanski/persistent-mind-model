...
PMM_BANDIT_REQUIRE_TARGETING_IN_HOT = (
    os.getenv("PMM_BANDIT_REQUIRE_TARGETING_IN_HOT", "true").lower() == "true"
)
...
def parse_targeted_reflection(
    reflection_text: str, active_commits: List[Dict]
    ...
    for line in lines:
        ...
        if line.lower().startswith("commit:"):
            ...
            for commit in active_commits:
                if (
                    commit.get("id", "") in commit_ref
                    or commit_ref in commit.get("title", "")
                    or commit_ref in commit.get("content", "")
                ):
                    targeted_commit_id = commit.get("id")
                    targeted_commit_title = commit.get(
                        "title", commit.get("content", "")
                    )[:60]
                    break
...
def enhance_reflection_telemetry(
    reflection_text: str, bandit_context: Dict, active_commits: List[Dict]
) -> Dict:
    ...
    if PMM_BANDIT_REQUIRE_TARGETING_IN_HOT and hot_strength >= PMM_BANDIT_HOT_STRENGTH:
...
def backfill_close_attribution(
    reflection_events: List[Dict], evidence_events: List[Dict], horizon: int = None
) -> None:
    ...
    for evidence in evidence_events:
        ...
        for reflection in reflection_events:
            ...
            if (
                targeted_commit == commit_ref
                and evidence_turn > reflection_turn
                and evidence_turn - reflection_turn <= horizon
            ):
...
"""
    ...
    telemetry = enhance_reflection_telemetry(
        reflection_text, bandit_context, active_commits
    )

    assert telemetry["targeted_commit_id"] == "commit_1"
    assert telemetry["has_next_directive"]
