...
class ModelSelector:
    ...
    def display_current_personality(self):
        ...
        print(
            f"   Events: {len(self.mgr.model.self_knowledge.autobiographical_events)}"
        )
        ...
        print(
            f"   Commitments: {commitment_metrics['commitments_open']} open, {commitment_metrics['commitments_closed']} closed"
        )
        ...
        print(
            f"     Conscientiousness: {big5.conscientiousness.score:.3f} (baseline: 0.500)"
        )
        ...
        if commitment_metrics["commitments_total"] > 0:
            print("   Recent Commitments:")
            for cid, commitment in list(
                self.mgr.commitment_tracker.commitments.items()
            )[-3:]:
    ...
    def test_model_with_personality(self, model_choice: str):
        ...
        try:
            ...
            if insight:
                ...
                new_commitment_metrics = (
                    self.mgr.commitment_tracker.get_commitment_metrics()
                )
                new_patterns = dict(self.mgr.model.self_knowledge.behavioral_patterns)

                print("\n📈 Updated State:")
                print(
                    f"   Total Insights: {len(self.mgr.model.self_knowledge.insights)}"
                )
                print(
                    f"   Total Commitments: {new_commitment_metrics['commitments_total']}"
                )
                ...
                big5 = self.mgr.model.personality.traits.big5
                if (
                    big5.openness.score != 0.500
                    or big5.conscientiousness.score != 0.500
                ):
                    print("   🧬 Personality Evolution Detected:")
                    print(f"      Openness: {big5.openness.score:.3f}")
                    print(
                        f"      Conscientiousness: {big5.conscientiousness.score:.3f}"
                    )
            ...
            if "OPENAI_API_KEY" in str(e):
                print(
                    "   💡 Set OPENAI_API_KEY environment variable to test OpenAI models"
                )
    ...
    def run_interactive_demo(self):
        ...
        print(
            "This demonstrates model-agnostic consciousness: personality independent of LLM backend.\n"
        )
