...
class TestModelPersistence:
    ...
    def test_thread_safety(self):
        ...
        def save_operation():
            try:
                with patch.object(self.persistence, '_to_dict', return_value={"thread": threading.current_thread().name}):
                    with patch('pmm.persistence.validate_model'):
                        self.persistence.save(model)
...
class TestModelPersistenceIntegration:
    ...
    def test_full_save_load_cycle(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with patch.object(persistence, '_to_dict', return_value={"name": "Test Agent"}):
                with patch('pmm.persistence.validate_model'):
