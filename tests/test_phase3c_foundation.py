...
def test_adaptive_trigger_logic():
    ...
    state = TriggerState(last_reflection_at=now - timedelta(days=2), events_since_reflection=4)
    trigger = AdaptiveTrigger(config, state)
    
    should_reflect, reason = trigger.decide(now, ias=0.5, gas=0.5, events_since_reflection=4)
    ...
    state = TriggerState(last_reflection_at=now - timedelta(days=2), events_since_reflection=2)
    trigger = AdaptiveTrigger(config, state)
    
    should_reflect, reason = trigger.decide(now, ias=0.5, gas=0.5, events_since_reflection=2)
    ...
    state = TriggerState(last_reflection_at=now - timedelta(hours=6), events_since_reflection=4)
    trigger = AdaptiveTrigger(config, state)
    
    should_reflect, reason = trigger.decide(now, ias=0.2, gas=0.2, events_since_reflection=4)
    ...
    state = TriggerState(last_reflection_at=now - timedelta(days=1.5), events_since_reflection=2)
    trigger = AdaptiveTrigger(config, state)
    
    should_reflect, reason = trigger.decide(now, ias=0.8, gas=0.8, events_since_reflection=2)
...
def test_semantic_analyzer_novelty():
    ...
    assert novelty_novel > novelty_similar, "Novel text should have higher novelty score"
    
    # Test duplicate detection
    is_duplicate = analyzer.is_semantic_duplicate(similar_text, reference_texts, threshold=0.8)
    ...
    reflections = [
        {
            ...
            "timestamp": (datetime.now(timezone.utc).replace(tzinfo=None) - timedelta(days=1)).isoformat(),
            "meta": {}
        },
        {
            "content": "My reflection quality seems to be getting better with practice",
            "timestamp": (datetime.now(timezone.utc).replace(tzinfo=None) - timedelta(hours=12)).isoformat(),
            "meta": {}
        },
        {
