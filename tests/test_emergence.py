...
def test_experience_query_detect():
    ...
    assert not analyzer.experience_query_detect(normal_text)

    # Empty text
    assert not analyzer.experience_query_detect("")
...
def test_novelty_score():
    ...
    events = [
        ...
        EmergenceEvent(
            3, "2024-01-03", "response", "Something completely different", {}
        ),
    ]
...
def test_stage_detection():
    ...
    # S0: Substrate
    stage = analyzer.detect_stage(
        IAS=0.1, GAS=0.1, exp_detect=False, pmmspec=0.1, selfref=0.01
    )
    assert stage == "S0: Substrate"

    # S1: Resistance
    stage = analyzer.detect_stage(
        IAS=0.3, GAS=0.2, exp_detect=False, pmmspec=0.4, selfref=0.1
    )
    assert stage == "S1: Resistance"

    # S2: Adoption
    stage = analyzer.detect_stage(
        IAS=0.6, GAS=0.3, exp_detect=False, pmmspec=0.7, selfref=0.4
    )
    assert stage == "S2: Adoption"

    # S3: Self-Model
    stage = analyzer.detect_stage(
        IAS=0.7, GAS=0.4, exp_detect=True, pmmspec=0.8, selfref=0.5
    )
    assert stage == "S3: Self-Model"

    # S4: Growth-Seeking
    stage = analyzer.detect_stage(
        IAS=0.8, GAS=0.7, exp_detect=True, pmmspec=0.9, selfref=0.6
    )
