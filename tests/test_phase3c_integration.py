...
def test_adaptive_reflection_triggers():
    ...
    try:
        ...
        os.environ['PMM_REFLECTION_CADENCE'] = '0.01'  # Very short cadence for testing (0.01 days = ~15 minutes)
        ...
        print(f"✅ PMM initialized with adaptive triggers")
        ...
        for i, user_input in enumerate(test_inputs):
            ...
            recent_events = pmm.storage_manager.recent_events(limit=5)
            reflection_events = [e for e in recent_events if e[2] == 'reflection']  # kind is at index 2
            
            if len(reflection_events) > reflection_count:
                reflection_count = len(reflection_events)
                print(f"🎯 Adaptive reflection triggered! Total reflections: {reflection_count}")
                
                # Print the reflection content
                latest_reflection = reflection_events[0]
                print(f"   Reflection: {latest_reflection[3][:100]}...")  # content is at index 3
        
        print(f"✅ Adaptive reflection test completed. Reflections triggered: {reflection_count}")
        
        # Test that we can query emergence data
        from pmm.emergence import EmergenceAnalyzer
        analyzer = EmergenceAnalyzer(storage_manager=pmm.storage_manager)
        
        ias, gas = analyzer.compute_scores()
        close_rate = analyzer.commitment_close_rate()
        
        print(f"✅ Emergence metrics: IAS={ias:.3f}, GAS={gas:.3f}, Close Rate={close_rate:.3f}")
...
def test_semantic_novelty_in_reflections():
    ...
    try:
        ...
        pmm = PersistentMindMemory(model_name="test-model", db_path=db_path)
        
        # Add some reflections manually to test semantic analysis
        store = pmm.storage_manager
        ...
        try:
            ...
            assert novelty2 > novelty1, f"Novel reflection should have higher novelty: {novelty2} vs {novelty1}"
            
            print("✅ Semantic novelty detection working correctly")
            return True
            
        except Exception as e:
            print(f"⚠️  Semantic analysis test skipped (requires embedding provider): {e}")
