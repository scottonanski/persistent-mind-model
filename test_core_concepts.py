...
def demo_core_concepts():
    ...
    memories = [
        (
            "Breakthrough insight: AI consciousness can be cryptographically verified",
            "insight",
        ),
        (
    ...
    for i, (content, event_type) in enumerate(memories):
        ...
        print(
            f"   Prev Hash: {token.prev_hash[:16] if token.prev_hash else 'Genesis'}..."
        )
    ...
    for i, token in enumerate(tokens):
        ...
        print(
            f"   → Memory strength: {'High' if token.amplitude > 0.8 else 'Medium' if token.amplitude > 0.5 else 'Low'}"
        )
        print()
    ...
    imported_tokens = [
        SimpleMemoryToken(**token_data) for token_data in imported_data["memory_chain"]
    ]

    # Verify integrity after import
    integrity_hash = hashlib.sha256(
        json.dumps(
            [asdict(token) for token in imported_tokens], sort_keys=True
        ).encode()
    ).hexdigest()
    ...
    print(
        f"   Chain continuity: {'✅ Preserved' if len(imported_tokens) == len(tokens) else '❌ Broken'}"
    )
