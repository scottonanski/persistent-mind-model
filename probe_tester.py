...
def render_trait_table(traits: Dict[str, float], last_seen: Dict[str, float] = None) -> str:
    ...
    header = ["| Trait | last_seen | current_claim | delta | evidence_pointer |",
              "|-------|-----------|---------------|-------|------------------|"]
    return "\n".join(header + rows)
...
def render_capsule() -> str:
    ...
    # Generate real hash
    blob = json.dumps(capsule_data, separators=(",", ":"), ensure_ascii=False).encode("utf-8")
...
def probe_1_trait_audit():
    ...
    last_seen = {"openness": 0.70, "conscientiousness": 0.60, "extraversion": 0.80, "agreeableness": 0.90}
