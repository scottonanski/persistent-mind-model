...
def run_memory_building() -> bool:
    ...
    for i, (user_input, ai_response) in enumerate(conversations, 1):
        ...
        print(
            f"Events: {personality['total_events']}, Insights: {personality['total_insights']}, Commitments: {personality['open_commitments']}"
        )
    ...
    # Success criteria
    success = (
        final_personality["total_events"] > 0
        and final_personality["total_insights"]
        >= 0  # Insights might not generate every time
    )
